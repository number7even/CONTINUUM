 ...
 *
 * Exposes 9 tools + 4 Resources + 2 Prompts to MCP-aware AI clients
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          aroundId: {
            type: 'string',
            description: 'Observation ID to anchor the timeline window on. Preferred when you ' +
              'have an ID from Layer-1 search. Mutually exclusive with `at`.',
          },
          at: {
            type: 'string',
            description: 'ISO-8601 timestamp to anchor on (e.g. "2026-05-15T12:13:41Z"). Use ' +
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          id: {
            type: 'string',
            description: 'Observation ID to permanently delete. Narrow this ID via continuum_search_docs or continuum_timeline first to make sure you are deleting the right row.',
          },
        },
        required: ['id'],
      },
...
server.setRequestHandler(CallToolRequestSchema, async request => {
  ...
  try {
    switch (name) {
      ...
      case 'continuum_get_observations': {
      ...
      case 'continuum_delete_observation': {
