...
| L4 talking-head | source face + audio → lip-synced mp4. Target = **`fal-ai/musetalk`** (master map) but **⚠️ G1: NO render backend wired/proven** (code-verified handover: only image models, no credential; fal/PR#11 not on disk; map vs handover disagree). Batch seam = `ReplicateService` poll or sovereign `AVATAR_SOVEREIGN_RENDER_URL`. Rented talent → VAULT (signed). | ❌ render backend OPEN (G1); not on the voice-over-b-roll MVP path |
| L5 matte | presenter cut-out — **build server-side RVM / BiRefNet / MediaPipe → FFmpeg `yuva420p`**; BodyPix is **NOT reusable** (handover G3: browser per-frame PNG, dated). Or VAULT alpha. **Hard prerequisite** before Riaan/Astrid faces re-enter L5. | 🅿️ content-team build (not VC reuse) |
