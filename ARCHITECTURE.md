...
- **V0 polish** (in progress 2026-05-20+):
  ...
  - ❌ remaining: `git` adapter, STATE.md → first-checkpoint parser, privacy-filter extensions per CTO doc §A3 (JWT shapes, GCP service-accounts, entropy detector, operator-extensible patterns config — base filter `<private>` + sk-/xai-/AKIA/PEM already shipped in `observation.ts:42`).
