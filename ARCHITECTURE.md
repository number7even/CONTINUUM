...
- **V0 polish** (in progress 2026-05-20+):
  ...
  - ❌ remaining: privacy-filter extensions per CTO doc §A3 (JWT shapes, GCP service-accounts, entropy detector, operator-extensible patterns config — base filter `<private>` + sk-/xai-/AKIA/PEM already shipped in `observation.ts:42`). This is the only remaining V0-polish item; everything else is shipped.
