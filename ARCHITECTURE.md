...
| **ID-1** | VoiceCosmos isolation boundary: org vs property | tenant-per-org / tenant-per-property | org | ✅ Locked 2026-08-17 — **CONTINUUM `tenantId` ≡ VoiceCosmos `tenant_id` (the ORG)**; `property_id` is first-class scoping metadata stamped on every Observation and filtered at retrieval (same intra-tenant layer as the deferred role-loadouts). Structural 404-isolation separates legal entities; cross-property visibility inside an org is org policy. Decided after the VC schema audit disproved 1:1 property=tenant (34 properties across 15 of 3,899 tenants). Mint on activation only. |
| **D-V2.2** | Hosted-fleet tenancy: data plane vs control plane | Postgres-only rewrite / hybrid (per-tenant SQLite+RuVector data plane + thin Postgres control plane) | hybrid — reconciliation (a) of the 2026-05-15 flag | ✅ Locked 2026-08-10 — **Hybrid Tenant Data Model.** (1) **Data plane, isolated per tenant:** each workspace keeps its path-safe SQLite file + RuVector index under `$CONTINUUM_DATA_DIR/<tenantId>/` (local `~/.continuum/`, Fly `/data/`) — preserves data sovereignty, keyless FTS5, physical compliance; no D2 revision needed. (2) **Control plane, unified:** a thin Postgres directory mapping `workspace_id` → storage node, OAuth profiles, billing webhooks, API quota. (3) **Metering:** write paths gate on control-plane subscription status (fail-safe on delinquency). Postgres never holds tenant content — it wraps the data plane as the auth/tenancy directory (the "common SaaS pattern" reconciliation (a) foreseen in the flag). |
...
- **V0 polish** (in progress 2026-05-20+):
  - ✅ **shipped 2026-05-23 (commit `31fe885`):** 3 additional Resources (`continuum://state/current`, `continuum://digest/latest`, `continuum://session/briefing`), 2 Prompts (`continuum.session_start`, `continuum.cite`), `agent_handoff` observation kind (V0-compatible RecursiveMAS intent capture, see Issue #3).
  - ✅ **shipped 2026-05-23:** CLI (`continuum init/start/status`) — single bin in `packages/cli`, hand-rolled argv, `init` prints MCP registration snippet, `start` execs the MCP server, `status` shows latest snapshot + todo counts.
  - ✅ **shipped 2026-05-23:** `packages/adapters/docs` — idempotent markdown ingester (`.md` / `.mdx`), stable per-file IDs from `sha256(relativePath)`, FTS5 verified. Adds `StorageBackend.upsertObservation()` primitive (interface + SQLite impl + observation.ts helper) so any adapter can ship idempotent re-syncs without raw-SQL access.
  - ✅ **shipped 2026-05-23:** `packages/adapters/git` — one Observation per commit with `type='commit'`, raw 40-char SHA as the stable ID (slice(0,8) = canonical git short-hash). Reads `git log -z --pretty=format` with `\x1f` field separators for safe parsing of multi-line bodies. Excludes diffs by design (token + privacy cost; recoverable via `git show`). Source-moat coverage is now 3-of-5 (`export` + `docs` + `git`); `mem` (claude-mem) and `sona` deferred to V0.5 per the original phasing.
  - ✅ **shipped 2026-05-24:** STATE.md → first-checkpoint parser. Pure string→struct parser in `packages/core/src/state-md.ts` (no I/O — trivially testable, reusable from MCP tools). Wired into `continuum init` (auto-imports when STATE.md present + no checkpoints yet) and exposed as `continuum import-state` for forced re-imports. Tolerant of malformed entries: drops to warnings rather than corrupting the snapshot. Smoke-tested on the real VC-Hospitality STATE.md (11 active + 3 dormant + 0 broken from 17 entries; 4 entries flagged for missing required fields).
  - ✅ **shipped 2026-05-24:** privacy-filter §A3 extensions. Pattern set expanded from 4 → 11 (added JWT, GCP service-account JSON, GitHub tokens, Slack tokens, Google API keys, Stripe live secret + publishable). Patterns now **actually scrub** the content (replace with `[REDACTED:<label>]`) instead of only being detected. Operator-extensible via JSON file at `$CONTINUUM_PRIVACY_CONFIG` (default `~/.continuum/privacy.json`). Optional Shannon-entropy detector gated by `CONTINUUM_PRIVACY_ENTROPY_DETECTOR=1` (threshold 4.5 bits/char — passes through hex commit SHAs at ~4.0 to avoid false positives). Verified via `scripts/privacy-smoke.mjs` (13 checks, all green; covers every pattern + the commit-SHA false-positive guard + operator config load + bad-config graceful degradation).
  - **V0 polish is now closed.** Every item from the original 2026-05-20 CTO-doc punch-list has shipped. V0.5 (RuVector migration) is the next major phase.
- **V0.5** (week 2–3): `claude-mem` + `sona` adapters, **ruv-FANN OR ruvllm for local digest** (§10a + §10b), **RuVector migration** if gates pass (§10b), Chroma backfill or skip-and-go-direct-to-RuVector decision.
  - ✅ **V0.5 stub shipped 2026-05-24:** Path A from Issue #20. `HybridStorageBackend` composes SQLite (relational) + RuVector @0.2.25 native (HNSW vector index) + `@xenova/transformers` MiniLM-L6-v2 (384-dim embeddings). Opt-in via `CONTINUUM_STORAGE_BACKEND=hybrid`; sqlite remains the default. Sync `StorageBackend` interface untouched — vector ops exposed as new async methods on `HybridStorageBackend` (`vectorSearch`, `flushVectorWrites`, `vectorCount`). 9-check smoke `scripts/ruvector-smoke.mjs` all green. Migration to default-on, hybrid FTS5+vector RRF search, and bulk-path vector indexing are V0.5 follow-up work.
- **V1** (month 1): Docker self-host, **MCP over HTTP/SSE**, Apache-2.0 OSS release, **ruv-swarm for per-adapter ephemeral source aggregation** (§10a), **RVF COW snapshots replace JSON checkpoints** (§10b), DSPy.ts integration begins ([Issue #1](https://github.com/number7even/CONTINUUM/issues/1)).
  - ✅ **V1 HTTP/SSE stub shipped 2026-05-24**: `packages/mcp-server/src/http.ts` — Express + `@modelcontextprotocol/sdk` `SSEServerTransport` + Bearer-token auth (`$CONTINUUM_HTTP_TOKEN`) + project routing (X-Continuum-Project header / ?project= query / env default). `/healthz` exempt for load-balancer probes. Per-session `buildServer(projectId)` instances so multi-tenant traffic doesn't share state. Opt-in via `continuum serve` CLI command. 7-check end-to-end smoke `scripts/http-smoke.mjs` round-trips a real SDK `SSEClientTransport`. mcp-server refactored: thin `index.ts` (42) + factory `server.ts` (783) + `http.ts` (163) — Issue #12 partially addressed. **Issue #8 closed** same turn (mandatory pre-HTTP gate): metadata deep-scrub via `scrubMetadataDeep` so adapters and remote agents can no longer bypass §8 by stuffing secrets into `Observation.metadata`. V1 follow-ups (TLS termination, OAuth, supervisor, containerization for npm-audit blast-radius mitigation) deferred to a follow-up issue.
  - ✅ **AaaS frontend scaffold shipped 2026-05-24**: `apps/console/` — Next.js 15 server-rendered page proves the Vercel ↔ CONTINUUM bridge. The Server Component connects to a running `continuum serve` engine via `SSEClientTransport`, opens an SSE session with Bearer auth, calls `listTools` + `listResources` + `listPrompts`, renders the live 7+4+2 registry. Vercel deployment: set Root Directory to `apps/console`, set `CONTINUUM_HTTP_URL` + `CONTINUUM_HTTP_TOKEN` env vars in Project Settings. `next build` clean. Engine itself stays off-Vercel (long-running daemon + native bindings); the frontend is the only thing on Vercel.
